...
class TestROStockCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        # activare EUR, USD, RON
        eur = cls.env.ref('base.EUR')
        eur.active = True
        usd = cls.env.ref('base.USD')
        usd.active = True
        ron = cls.env.ref('base.RON')
        ron.active = True
        ...
        cls.account_valuation.is_for_stock_valuation = True
        if cls.account_valuation.code != "371000":
            cls.account_valuation =  cls.env.company.account_stock_valuation_id.copy(
                {"code": "371000"}
            )

        if cls.account_expense.code != "371000":
            cls.account_expense =  cls.env.company.expense_account_id.copy(
                {"code": "607000"}
            )

        cls.account_valuation.copy(  {"code": "371001"}  )
        cls.account_expense.copy({"code": "607001"})

        new_stock_val_account1 = cls.account_valuation.copy(
            {  "code": "371002"  }
        )
        new_expense_acc1 =  cls.account_expense.copy(
            {"code": "607002" }
        )
        ...
        for product_ref, check_list in checks.items():
            ...
            for vals in check_list:
                ...
                if self.log_checks:
                    ...
                    # Antetul tabelului
                    _logger.info(
                        "%-5s | %-20s | %-10s | %-10s | %-5s | %-10s | %-10s | %-10s | %s",
                        "ID","Name","From","To","Qty",  'Value',  'Remain Qty', 'Price Unit',  'Remain Value'
                    )
                    _logger.info("-" * 120)

                    for move in product_moves:
                        _logger.info(
                            "%-5s | %-20s | %-10s | %-10s | %5.2f | %10.2f | %10.2f | %10.2f | %10.2f ",
                            move.id,
                            move.display_name,
