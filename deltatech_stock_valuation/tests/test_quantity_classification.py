...
class TestQuantityClassification(AccountTestInvoicingCommon):
    ...
    def test_negative_quantity_correction(self):
        """Convenția 18 pentru corecții/retururi pe note entry: aceeași parte,
        cantitate negativă (pattern validat pe baze de client) — intrare negativă."""
        self._post_entry(debit=1000.0, quantity=10.0)
        # corecție: +200 lei pe debit cu cantitate -2 (ex. diferență de preț cu retur cantitativ)
        move = self.env["account.move"].create(
            {
                "journal_id": self.journal.id,
                "line_ids": [
        ...
        history = self._get_history()
        self.assertTrue(history)
        self.assertEqual(history.quantity_in, 8.0, "corecția cu qty negativ pe debit e intrare negativă")
        self.assertEqual(history.quantity_out, 0.0)
        self.assertEqual(history.quantity_final, 8.0)
        self.assertEqual(history.amount_final, 1200.0)
