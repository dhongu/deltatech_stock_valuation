...
class ResConfigSettings(models.TransientModel):
    ...
    def _compute_refresh_valuation_step_info(self):
        ...
        if last_step and last_run:
            ...
            progress = self.env._("No background refresh has run yet.")
    ...
    def action_recompute_in_background(self):
        ...
        if cron and cron.sudo().active:
            return self._notification(
                self.env._("Background Recompute"),
                self.env._("A background recompute is already running."),
                "warning",
            )
    ...
    def start_auto_refresh(self):
        ...
        if cron:
            ...
            cron.sudo()._trigger()
        return self._notification(
            self.env._("Auto Refresh"),
            self.env._("Auto refresh started. It will run every 2 minutes until complete."),
            "success",
        )
